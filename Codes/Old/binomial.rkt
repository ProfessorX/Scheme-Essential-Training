...
(define (binom-test)
  (let ((nTests  0)
        (nPassed 0))
    (letrec ((test (lambda (exp str target)
                    (set! nTests (+ nTests 1))
                    (if (= exp target) 
                        (set! nPassed (+ nPassed 1))
                        (begin (display "  Error: ") (display str) (display " = ") 
                               (display exp) (display " != target.\n"))))))
      (begin
        ...
        (display "  SUMMARY: passed tests = ") (display nPassed)
        (display ", failed tests = ") (display (- nTests nPassed)) (display "\n")))
    (= nTests nPassed)))
